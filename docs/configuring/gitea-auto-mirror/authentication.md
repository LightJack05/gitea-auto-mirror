...
To protect the password and avoid leaking it via the containers env variables, secrets, etc., you can specify a hash instead. This hash will be used to validate incoming requests containing the plaintext password.

### Generate a hash
On a Linux or POSIX-Compliant machine, make sure you have `bash`, `openssl` and `argon2` installed and in your path.
If you have, run the following command in your shell:
